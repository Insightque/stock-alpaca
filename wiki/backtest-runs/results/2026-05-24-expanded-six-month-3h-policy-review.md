---
id: 2026-05-24-expanded-six-month-3h-policy-review
created_at: 2026-05-24T16:14:34+09:00
source_type: six-month-3h-policy-simulation
paper: true
orders_submitted: 0
---

# 확장 universe 최근 6개월 3시간 단위 시뮬레이션과 정책 검토

## 목적

사용자 요청에 따라 기존 관심 종목 외 다양한 업종의 미국 주식으로 universe를 확장하고, 2025-11-24부터 2026-05-22까지 최근 6개월 미국 정규 거래일을 대상으로 Alpaca MCP에서 30분봉을 읽어 정규장 3시간 구간으로 집계했다. 주문 제출, 취소, 교체, 포지션 변경은 하지 않았다.

기준 universe는 기존 관심 종목/보유 종목에 빅테크, 소프트웨어, 반도체 장비, 금융, 헬스케어, 소비재, 산업재, 에너지, 소재, 유틸리티, 고변동 성장주를 더한 62개 심볼이다. 전체 목록은 `AAPL, ABBV, ADBE, AMAT, AMD, AMZN, ASML, AVGO, BA, BAC, CAT, COIN, COP, COST, CRM, CVX, ETN, FCX, GE, GOOGL, GS, HD, HON, HOOD, INTC, IONQ, ISRG, JNJ, JPM, KLAC, LIN, LLY, LRCX, MA, MCD, META, MRK, MS, MSFT, MU, NEE, NKE, NOK, NVDA, ORCL, PFE, PLTR, QBTS, QQQ, RGTI, RTX, SBUX, SLB, SMH, SO, SPY, TSLA, TSM, UNH, V, WMT, XOM`이다.
...
- 3시간 구간 레코드: 22940개
- 정규장 집계 일봉 레코드: 7688개
- 원천: [[2026-05-24-expanded-six-month-3h-simulation-sources]]
- 계산 데이터: `wiki/evidence-store/sources/2026-05-24-expanded-six-month-3h-simulation-data.json`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 3h-momentum-top3 | 57 | 169 | 45.0% | 57 | 19 | $-150.28 | $-0.89 | 44.7% | $+1,248.04 |
| 3h-momentum-top2 | 57 | 113 | 45.1% | 40 | 16 | $+534.19 | $+4.73 | 45.6% | $+1,246.44 |
| 3h-vwap-reclaim-top2 | 54 | 87 | 42.5% | 45 | 14 | $-580.17 | $-6.67 | 39.5% | $-345.72 |
| 3h-afternoon-continuation-top2 | 52 | 104 | 61.5% | 14 | 26 | $+2,285.36 | $+21.97 | 68.5% | $+1,639.15 |

해석:

- 가장 좋은 총손익 정책은 `3h-afternoon-continuation-top2`였고, 전체 가상 P/L은 $+2,285.36, 거래당 평균은 $+21.97였다.
- 단순 3시간 모멘텀 top3는 종목 수를 늘리자 전체 손익이 $-150.28로 악화됐다. 후보 확장은 단타에서 잡음도 늘리므로 top2 제한이나 후속 유지 필터가 필요하다.
- `3h-vwap-reclaim-top2`는 확장 universe에서 $-580.17로 나빠져 단독 보조 신호로도 약해졌다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| daily-3h-quality-top5 | 84 | 320 | 162/320 | 174/320 | +8.76% | +6.73% | -8.56% | +9.30% |
| daily-3h-theme-capped-top5 | 84 | 320 | 174/320 | 186/320 | +9.68% | +7.65% | -9.07% | +11.84% |
| daily-3h-momentum-top3 | 84 | 192 | 86/192 | 103/192 | +10.33% | +8.30% | -10.41% | +12.71% |

해석:

- 전체 기준 평균 SPY 초과수익이 가장 높은 장타형 정책은 `daily-3h-momentum-top3`였다.
- 하지만 `daily-3h-momentum-top3`는 평균 불리 이동이 -10.41%로 가장 크고, INTC/AMD/NOK 같은 소수 성과에 집중됐다.
- `daily-3h-theme-capped-top5`는 기존 16종목 universe 대비 평균 SPY 초과가 +7.82%p에서 +7.65%p로 거의 유지됐고, 검증 구간은 +11.10%p에서 +11.84%p로 개선됐다. 확장 universe에서도 theme cap은 유지할 가치가 있다.
- 단순 `quality_top5`는 평균 SPY 초과가 +7.70%p에서 +6.73%p로 낮아졌다. 관심 종목을 무작정 늘리기보다 업종/테마 제한과 과열 제한을 같이 써야 한다.

## 기존 관심 종목 universe와 비교

| 정책 | 기존 16종목 | 확장 62종목 | 해석 |
| --- | ---: | ---: | --- |
| `3h-momentum-top3` P/L | $+981.43 | $-150.28 | 종목 확장 후 단타 추격 잡음 증가 |
| `3h-momentum-top2` P/L | $+517.95 | $+534.19 | top2 제한은 거의 유지 |
| `3h-vwap-reclaim-top2` P/L | $+734.04 | $-580.17 | 평균회귀 보조 신호는 확장 universe에서 폐기 쪽 |
| `3h-afternoon-continuation-top2` P/L | $+1,530.61 | $+2,285.36 | 오후 후속 유지 신호는 확장 후에도 가장 견조 |
| `daily-3h-quality-top5` 평균 SPY 초과 | +7.70%p | +6.73%p | 무차별 확장은 장타 품질을 약간 희석 |
| `daily-3h-theme-capped-top5` 평균 SPY 초과 | +7.82%p | +7.65%p | theme cap은 확장 universe에서도 안정적 |
| `daily-3h-momentum-top3` 평균 SPY 초과 | +10.62%p | +8.30%p | 강하지만 불리 이동과 집중 리스크 증가 |
...
| universe 확장 원칙 | 종목 수를 늘리면 단타와 단순 quality 정책의 잡음도 늘어난다. 확장 universe는 `theme cap + overheat guard + SPY/QQQ 상대강도`를 붙인 장타 후보 선별에 우선 사용하고, 단타에는 top2/후속 유지 확인 없이는 쓰지 않는다. |
...
- 확장 universe는 장타 후보 발굴에는 사용하되, 정책 점수 상위권에 반복 등장한 `INTC`, `NOK`, `AMD` 같은 종목은 성과 집중이므로 실적/filing/valuation 검증과 과열 제한을 통과해야 한다.
