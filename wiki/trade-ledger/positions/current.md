...
_Last updated: 2026-06-05 11:33 KST_

## 계좌 요약

- Alpaca paper account status: ACTIVE
- Portfolio value: $102,510.35
- Cash: $30,487.94
- Buying power: $252,931.58
- Long market value: $72,022.41

## 최신 after-hours-autopilot reconciliation

- Run: [[2026-06-05-1131-after-hours-autopilot]]
- Open/new: 없음
- Filled: 없음
- Cancelled: 없음
- Position count observed by Alpaca MCP: 33 positions. 장외 세션에서 신규 fill, 신규 open order, 포지션 수량 변화는 없었다.
- Recent reconciliation scope: scheduler-owned `1131` Alpaca core/research preflight와 runtime `get_clock`, `get_account_info`, `get_all_positions`, `get_orders(status=open)`, `get_orders(after=2026-06-04T20:00:00Z)`, `get_stock_snapshot` 기준 regular market closed와 장외 `ah-` 신규 주문 0건을 재확인했다. `QQQ`는 여전히 1주 ask가 per-order cap을 넘었고 scheduler clock 기준 quote freshness도 342.94분 stale라 `place_stock_order` 호출은 생략했다.
- Orders submitted/replaced/cancelled/closed by this workflow: 0 / 0 / 0 / 0.
- Source note: `wiki/trade-ledger/positions/2026-06-05-1131-after-hours-autopilot-post-trade.json`
...
- 위 계좌 요약 수치는 `1131` scheduler core preflight account snapshot과 runtime `get_clock/get_account_info/get_all_positions/get_orders(status=open)/get_orders(after)/get_stock_snapshot` 보조 확인 기준이다.
