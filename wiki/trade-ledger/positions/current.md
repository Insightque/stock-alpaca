...
_Last updated: 2026-06-16 06:16 KST_

## 최신 after-hours-autopilot reconciliation

- Run: [[2026-06-16-0611-after-hours-autopilot]]
- Open/new: 없음. 이번 cycle에서는 `place_stock_order`를 호출하지 않았고 신규 `client_order_id`도 만들지 않았다.
- Filled: 없음
- Cancelled: 없음
- Position count observed by Alpaca MCP: scheduler-owned `0611` core preflight와 direct `get_account_info/get_all_positions/get_watchlists/get_account_activities(activity_types=FILL, after=2026-06-15T20:00:00Z)` continuity 기준 account `ACTIVE`, positions `33`건, watchlists `0`건, same-session after-hours fills `0`건이었다.
- Recent reconciliation scope: scheduler-owned `0611` core/research preflight를 source-of-record로 사용했고 Alpaca core `first_blocking_gate=market_closed`는 after-hours expected nonblocking으로 처리했다. direct `get_stock_latest_quote(feed=iex, overnight)`와 `get_stock_snapshot(feed=iex)` readback 기준 freshest `QQQ`도 약 `25.67`분 stale였고 `RGTI/NOK/AVGO/PFE/MSFT/TSLA/SLB/GE`는 spread 또는 freshness cap을 동시에 위반해 submit path가 열리지 않았다.
- Orders submitted/replaced/cancelled/closed by this workflow: 0 / 0 / 0 / 0.
- Source note: `wiki/trade-ledger/positions/2026-06-16-0611-after-hours-autopilot-post-trade.json`
