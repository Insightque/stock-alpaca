...
| Market clock | PASS | scheduler core preflight `2026-06-18T10:51:11.84129075-04:00`, regular market open |
...
| Quote freshness | PASS | source-of-record quote timestamp `2026-06-18T14:51:31.499726341Z`, decision time 기준 약 `0.0`분 |
| Spread | MIXED | `RGTI 0.0976%`, `SO 0.0213%` PASS, buy fallback은 backlog throttle로 미진입 |
| Risk plan | PASS | `RGTI sell 5 @ 20.49 USD` trim plan 작성 |
...
| Symbol | 판단 | Spread | 이유 |
| --- | --- | ---: | --- |
| RGTI | executable_sell_trim | 0.0976% | speculative loss-control trim trigger, negative expected excess, open-order clean |
| SO | blocked_sell_metric_gap | 0.0213% | quote/spread는 pass지만 trim decision-grade expected-excess/replacement margin 공백 |
| AAPL | blocked_review_backlog_throttle | 0.0133% | buy-only backlog throttle로 신규 add 차단 |
...
| NVDA | blocked_review_backlog_and_same_theme_warning | 0.2959% | buy-only backlog throttle와 ai_semiconductor warning context가 겹침 |
...
- Planned orders: 1
- Submitted orders: 1
- Filled orders: `RGTI sell 5 @ limit 20.49 USD`, `filled_avg_price=20.812 USD`, `filled_at=2026-06-19 00:00:14 KST`
- Post-trade reconciliation: `get_order_by_client_id` 기준 `status=filled`, `filled_qty=5`, `get_orders(status=open)` 기준 open orders `0`, `get_all_positions` 기준 positions `33`, `RGTI qty=15`, `get_account_info` 기준 cash `28,301.48 USD`, portfolio value `101,683.39 USD`, buying power `303,198.26 USD`, `get_watchlists` 기준 watchlists `0`
...
| `check-risk-policy.py --json` | PASS | `RGTI sell 5 @ 20.49 USD` trim plan |

## 산출물

- Report: `wiki/current-runs/daily/2026-06-18-2351-hourly-autopilot.md`
- Manifest: `wiki/evidence-store/run-manifests/2026-06-18-2351-hourly-autopilot.json`
- Order plan: `wiki/trade-ledger/orders/2026-06-18-2351-hourly-autopilot.json`
- Runtime gate evaluation: `wiki/evidence-store/sources/2026-06-18-2351-hourly-autopilot-runtime-gate-evaluation.json`
- Post-trade snapshot: `wiki/trade-ledger/positions/2026-06-18-2351-hourly-autopilot-post-trade.json`
