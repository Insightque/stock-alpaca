# 2026-07-22-0851-after-hours-autopilot
...
- 결과: scheduler-owned `0851` core/research preflight를 source-of-record로 사용했고 Alpaca core `first_blocking_gate=market_closed`는 after-hours expected nonblocking으로 처리했다. 이어서 live Alpaca MCP continuity로 regular market closed, account `ACTIVE`, positions `32`, open orders `0`, same-session after-hours submitted orders `0`, fills `0`를 다시 확인했다. 그러나 freshest executable IEX quote가 `NOK` `2026-07-21T20:59:59Z`로 약 `173.94`분 stale였고 `QQQ/SPY`는 spread pass에도 1주 notional cap을 넘었으며 나머지 executable set은 stale quote 또는 spread 또는 quote completeness gate를 통과하지 못해 `place_stock_order`는 호출하지 않았다.

## 사용한 스케줄러 프리플라이트

- Alpaca core: `wiki/evidence-store/sources/2026-07-22-0851-after-hours-autopilot-alpaca-core-preflight.json`
- Research MCP: `wiki/evidence-store/sources/2026-07-22-0851-after-hours-autopilot-research-mcp-preflight.json`
- Gate evaluation: `wiki/evidence-store/sources/2026-07-22-0851-after-hours-autopilot-after-hours-gate-evaluation.json`
- Alpaca core preflight의 `first_blocking_gate=market_closed`는 after-hours 워크플로우에서 expected nonblocking으로 처리했다. 이번 cycle의 run id는 KST scheduler 파일명 `2026-07-22-0851-*`를 유지하지만, 실제 미국 장 판단과 continuity는 `2026-07-21 EDT` clock 기준으로 기록했다.

## Alpaca MCP 확인

- Regular market: closed. live `get_clock` 기준 `timestamp=2026-07-21T19:53:55.752991512-04:00`였다.
- Account: live `get_account_info` 기준 account `ACTIVE`, portfolio value `98605.89 USD`, cash `28610.95 USD`, buying power `297957.99 USD`였다.
...
- Quote boundary: live `get_stock_latest_quote(feed=iex)` 기준 freshest executable shortlist quote는 `NOK`였지만 약 `173.94`분 stale였다. `QQQ`는 약 `182.28`분 stale, `SPY`는 약 `233.59`분 stale였다. `feed=overnight` cross-check는 전 종목에서 `2026-07-21T08:00:00Z` 부근 older quote만 제공해 freshness gate를 다시 열지 못했다.

## 후보 평가

- `AVGO`: allowed sell side trim/exit를 우선 검토했지만 latestQuote `362.15/402.10`이 약 `233.89`분 stale였고 spread가 약 `9.94%`였다. held qty가 `1`주뿐이라 floor-size trim 후 잔여 최소수량 discipline도 만족하지 못했다.
- `SO`: latestQuote `89.08/99.75`가 약 `233.93`분 stale였고 spread가 약 `10.70%`였다. latest review backlog context상 `sell_metric_gap`도 남아 있어 decision-grade trim으로 승격되지 않았다.
- `QQQ`: benchmark fallback으로 검토했지만 latestQuote `708.17/708.24`가 약 `182.28`분 stale였다. spread 자체는 약 `0.01%`로 양호했지만 1주 ask `708.24 USD`가 after-hours per-order notional cap `493.03 USD`를 넘었다.
- `SPY`: benchmark fallback secondary 후보였지만 latestQuote `748.21/748.32`가 약 `233.59`분 stale였고 1주 ask `748.32 USD`도 same per-order cap을 넘었다.
- `NOK`: latestQuote `10.78/10.92`가 약 `173.94`분 stale였고 spread도 약 `1.28%`로 cap 초과였다. `review-due-index`의 `blocked_add_symbols=['NOK']`도 add path를 계속 막고 있었다.
...
- Manifest: `wiki/evidence-store/run-manifests/2026-07-22-0851-after-hours-autopilot.json`
- Order plan: `wiki/trade-ledger/orders/2026-07-22-0851-after-hours-autopilot.json`
- Gate evaluation: `wiki/evidence-store/sources/2026-07-22-0851-after-hours-autopilot-after-hours-gate-evaluation.json`
- Post-trade check: `wiki/trade-ledger/positions/2026-07-22-0851-after-hours-autopilot-post-trade.json`
- Report: `wiki/current-runs/daily/2026-07-22-0851-after-hours-autopilot.md`
