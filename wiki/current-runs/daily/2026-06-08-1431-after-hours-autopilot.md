# 2026-06-08-1431-after-hours-autopilot
...
- 결과: scheduler-owned `1431` core/research preflight를 우선 사용했고 Alpaca core `first_blocking_gate=market_closed`는 after-hours expected nonblocking으로 처리했다. 이번 cycle은 same-session after-hours order budget이 이미 `2/2`라 submit 없이 종료했다.

## 사용한 스케줄러 프리플라이트

- Alpaca core: `wiki/evidence-store/sources/2026-06-08-1431-after-hours-autopilot-alpaca-core-preflight.json`
- Research MCP: `wiki/evidence-store/sources/2026-06-08-1431-after-hours-autopilot-research-mcp-preflight.json`
...
- Regular market: closed (runtime `get_clock` `2026-06-08T01:32:48.328534309-04:00`)
- Account/positions: runtime `get_account_info` 기준 account `ACTIVE`, portfolio value `98,287.30 USD`, cash `30,731.86 USD`, buying power `295,796.81 USD`, positions `33`건이었다.
...
- `AVGO` sell/trim은 runtime overnight quote `385.97/386.55`, spread `0.150124%`, quote age 약 `0.81`분으로 executable sell path를 유지했다.
- `GOOGL`, `TSLA`, `BAC`, `PFE`, `WMT`, `XOM`는 runtime overnight quote/freshness/spread/notional cap을 통과해 fallback buy executable stack을 형성했다.
- `QQQ`, `SPY`, `SMH`는 1주 ask가 after-hours per-order notional cap `491.44 USD`를 초과했다. `SMH`는 spread `0.332601%`도 cap을 넘겼다.
...
- Order plan: `wiki/trade-ledger/orders/2026-06-08-1431-after-hours-autopilot.json`
- Run manifest: `wiki/evidence-store/run-manifests/2026-06-08-1431-after-hours-autopilot.json`
- Gate evaluation: `wiki/evidence-store/sources/2026-06-08-1431-after-hours-autopilot-after-hours-gate-evaluation.json`
- Post-trade snapshot: `wiki/trade-ledger/positions/2026-06-08-1431-after-hours-autopilot-post-trade.json`
- Deterministic submit artifact: `wiki/evidence-store/sources/2026-06-08-1431-after-hours-autopilot-deterministic-submit.json`
