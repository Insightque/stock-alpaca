# 2026-05-30-1231-after-hours-autopilot
...
- Regular market not open: PASS. Alpaca MCP clock은 `2026-05-29T23:33:03.610205788-04:00` 기준 `is_open=false`다.
- Alpaca core: PASS for after-hours interpretation. Scheduler preflight의 `first_blocking_gate=market_closed`는 장외 workflow에서 예상된 비차단 상태로 처리했다. Scheduler account/positions/open orders/assets/quotes rows를 사용했고, runtime Alpaca MCP는 clock/account/positions/open orders/QQQ asset/overnight quote를 확인했다.
...
- Quote freshness: FAIL. Runtime overnight quotes are timestamped around `2026-05-29T08:00:00Z`, exceeding the after-hours 5-minute cap.
- Spread: MIXED. QQQ/NVDA는 after-hours spread 한도 이내지만 나머지는 한도 밖이다.
- Risk validator: PASS. 빈 주문 계획이므로 `orders is empty` warning만 발생했다.
- Order-plan schema mismatch: `AGENTS.md`는 top-level `quote_captured_at`/`asset_checked_at`를 요구하지만 현재 `harness/order-plan.schema.json`은 해당 top-level fields를 거부한다. Validator source of truth를 따라 plan에는 넣지 않고 이 mismatch를 log에 남긴다.
...
| AVGO | false | false | skip | available quote evidence exceeded after-hours 5-minute freshness cap; AI semiconductor concentration blocks after-hours add. |
| NVDA | false | true | skip | available quote evidence exceeded after-hours 5-minute freshness cap; AI semiconductor concentration blocks after-hours add. |
...
- Manifest: `wiki/evidence-store/run-manifests/2026-05-30-1231-after-hours-autopilot.json`
- Order plan: `wiki/trade-ledger/orders/2026-05-30-1231-after-hours-autopilot.json`
- Runtime Alpaca spot check: `wiki/evidence-store/sources/2026-05-30-1231-after-hours-autopilot-runtime-alpaca-spot-check.json`
- Scheduler Alpaca preflight: `wiki/evidence-store/sources/2026-05-30-1231-after-hours-autopilot-alpaca-core-preflight.json`
- Scheduler research preflight: `wiki/evidence-store/sources/2026-05-30-1231-after-hours-autopilot-research-mcp-preflight.json`
