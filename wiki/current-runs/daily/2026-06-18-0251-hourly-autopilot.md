...
| Market clock | PASS | live `get_clock` `2026-06-17T13:53:12.001560578-04:00`, regular market open |
| Stale order lifecycle | PASS | `0251` stale cleanup artifact 기준 stale candidates `0`, remaining open orders `0` |
| Alpaca core MCP | PASS | scheduler core preflight hard gate `pass`, direct continuity 기준 open orders `0`, account `ACTIVE`, `0231` `SLB` fill 반영 |
...
| RGTI | blocked_same_day_duplicate_sell | 0.0471% | speculative trim trigger와 spread는 pass지만 same US-date after-hours trim fill 때문에 추가 sell 차단 |
...
| SLB | blocked_same_day_duplicate_buy | 0.0392% | `0231` buy 1주가 same US-date에 이미 filled |
| SPY | blocked_validation_floor_cap | 0.0067% | 1주 ask `750.44 USD`가 validation floor per-order cap 약 `507.22 USD`를 초과 |
| QQQ | blocked_validation_floor_cap | 0.0068% | 1주 ask `733.58 USD`가 validation floor per-order cap 약 `507.22 USD`를 초과 |
| INTC | blocked_ai_semiconductor_warning_band | 0.0082% | ai_semiconductor theme/factor/cluster warning band 위라 relaxed floor-size buy 승격 중단 |
| AVGO | blocked_ai_semiconductor_warning_band | 0.0302% | same ai_semiconductor complex warning band와 post-earnings risk watch 지속 |
| PLTR | blocked_low_source_confidence | 0.0221% | current ticker note가 `이번 주문 제외`, `신뢰도: 낮음`을 유지해 floor-size add hard gate 미충족 |
| JPM | blocked_live_spread_and_missing_current_cycle_research_confirmation | 5.9593% | spread hard cap fail, plus current-cycle scheduler research preflight symbol scope 밖 |
| V | blocked_live_spread_and_missing_current_cycle_research_confirmation | 1.4384% | spread hard cap fail, plus current-cycle scheduler research preflight symbol scope 밖 |
| JNJ | blocked_live_spread_and_missing_current_cycle_research_confirmation | 5.3551% | spread hard cap fail, plus current-cycle scheduler research preflight symbol scope 밖 |
...
- Post-trade reconciliation: 이번 cycle 신규 submit attempt는 없었다. direct `get_orders(status=open)` 기준 open orders `0`, `get_account_info/get_all_positions` 기준 cash `28,324.87 USD`, portfolio value `101,444.24 USD`, buying power `302,179.65 USD`, positions `33`, `SLB qty=8`, `SO qty=6`, `RGTI qty=27`, `PFE qty=2`를 재확인했다.
...
- Report: `wiki/current-runs/daily/2026-06-18-0251-hourly-autopilot.md`
- Manifest: `wiki/evidence-store/run-manifests/2026-06-18-0251-hourly-autopilot.json`
- Order plan: `wiki/trade-ledger/orders/2026-06-18-0251-hourly-autopilot.json`
- Runtime gate evaluation: `wiki/evidence-store/sources/2026-06-18-0251-hourly-autopilot-runtime-gate-evaluation.json`
...
- `validation floor per-order cap`: floor-size learning buy도 `portfolio_value * 0.5%` 상한을 넘을 수 없다. 이번 cycle cap은 약 `507.22 USD`다.
