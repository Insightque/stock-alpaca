---
id: 2026-05-26-2124-hourly-autopilot
created_at: 2026-05-26T12:24:09Z
timezone: Asia/Seoul
paper: true
...
# 2026-05-26 21:24 KST hourly paper autopilot

## 결론

- 이번 run은 `blocked`다.
- `.env`의 `ALPACA_PAPER_TRADE=true`는 확인했다.
- Alpaca MCP로 포지션, expanded universe 일봉, asset checks, market movers, Alpaca news, 일부 historical quotes는 확인했다.
- 하지만 Alpaca clock/account/open orders/fills/watchlists/latest quote/snapshot/latest bar가 usable하지 않았다.
- 주문 후보인 `LLY`, `LRCX`, `ASML`, `AAPL`, `SMH`의 최근 20분 quote가 비어 있어 fresh quote와 spread를 검증하지 못했다.
...
| Alpaca market clock | FAIL | `get_clock` unusable/cancelled |
...
| Quote freshness | FAIL | 후보별 recent quote 응답 없음 |
| Spread | FAIL | 후보별 bid/ask/spread 없음 |
...
- `get_all_positions` 기준 long market value는 57,234.123 USD, 총 미실현 손익은 약 1,264.703 USD다.
...
| --- | ---: | ---: | --- | --- |
| LLY | 1004.695 | 1065.60 | 상승 | 백신 개발사 인수 및 VERVE-102 headline, quote/spread 부재 |
| LRCX | 284.37 | 305.43 | 상승 | semicap momentum, 이미 보유 중 |
| ASML | 1501.52 | 1632.98 | 상승 | semicap/ADR momentum, research MCP 부재 |
| AAPL | 300.19 | 308.81 | 상승 | mega-cap 후보이나 catalyst 혼재 |
| SMH | 556.85 | 575.91 | 상승 | 반도체 ETF, 기존 AI 반도체 cluster와 중복 |
| SPY | 739.10 | 745.67 | 상승 | benchmark |
| QQQ | 708.91 | 717.49 | 상승 | benchmark |

Quote freshness와 spread는 주문 후보에서 확인하지 못했다. 이 이유만으로도 submit은 금지된다.
...
| --- | --- | --- | --- | --- | --- |
| Alpaca | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | positions/bars/assets/news 일부는 성공했지만 clock/account/orders/fills/quotes/spread가 실패 |
| SEC EDGAR | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | direct MCP calls cancelled after retry |
| Alpha Vantage | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | direct MCP `TOOL_LIST` cancelled after retry |
| FRED | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | local MCP DNS failure to `api.stlouisfed.org` |
| Firecrawl | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | local MCP DNS failure to `api.firecrawl.dev` |
| Yahoo Finance | true | false | failed | [[2026-05-26-2124-hourly-autopilot-sources]] | direct MCP calls cancelled |
...
| 4 | AAPL | 61 | low | 대형주 안정성은 있으나 catalyst quality가 혼재하고 final candidate 제외 |
...
- `PATH=/usr/local/bin:$PATH python3 scripts/check-universe-coverage.py --strict wiki/evidence-store/run-manifests/2026-05-26-2124-hourly-autopilot.json`: PASS
- `PATH=/usr/local/bin:$PATH python3 scripts/check-mcp-coverage.py --strict wiki/evidence-store/run-manifests/2026-05-26-2124-hourly-autopilot.json`: FAIL
- `PATH=/usr/local/bin:$PATH python3 scripts/check-risk-policy.py --json wiki/trade-ledger/orders/2026-05-26-2124-hourly-autopilot.json`: PASS with `orders is empty`

## 산출물

- Raw source: [[2026-05-26-2124-hourly-autopilot-sources]]
- Manifest: `wiki/evidence-store/run-manifests/2026-05-26-2124-hourly-autopilot.json`
...
- `spread`: bid/ask 차이를 중간가격으로 나눈 비율이다. 이번 run은 후보별 bid/ask가 없어 측정 불가다.
