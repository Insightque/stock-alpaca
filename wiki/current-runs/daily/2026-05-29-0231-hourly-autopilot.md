# 2026-05-29 02:31 KST hourly autopilot

## 실행 요약

- Workflow: `harness/workflows/hourly-autopilot.md`
- Run ID: `2026-05-29-0231-hourly-autopilot`
- Paper mode: `ALPACA_PAPER_TRADE=true`
- Market clock: Alpaca scheduler preflight 기준 `2026-05-28T13:31:17.583329878-04:00`, regular session open.
...
| Stale order cleanup | PASS | `wiki/evidence-store/sources/2026-05-29-0231-hourly-autopilot-stale-order-cleanup.json`; stale candidate 0, remaining open orders 0 |
| Alpaca core | PASS | `wiki/evidence-store/sources/2026-05-29-0231-hourly-autopilot-alpaca-core-preflight.json`; clock/account/positions/open orders/recent activities/quotes pass |
...
| Quote/spread | PASS | Alpaca core preflight quote rows가 decision time 기준 20분 이내 |
| Risk | PASS for empty plan | 신규 주문 예산 `20/20`으로 주문 후보를 계획하지 않음 |

## 추천 후보와 차단 사유

Scheduler research preflight shortlist는 `AAPL`, `QQQ`, `SPY`, `GOOGL`, `NVDA`, `FCX`, `WMT`, `COP`, `SLB`, `BAC`, `TSLA`, `SO`였다. 이 중 universe와 MCP tier는 통과했지만, regular-session daily new-order cap이 이미 소진되어 모든 신규 validation buy를 차단했다.

Top recheck candidates: `AAPL`, `QQQ`, `SPY`, `GOOGL`, `NVDA`. 다음 regular-session ET 거래일 또는 daily order budget reset 이후 재평가한다. 같은 세션 내 relax candidate는 없다. 예산 제한은 hard risk gate라 완화하지 않는다.
