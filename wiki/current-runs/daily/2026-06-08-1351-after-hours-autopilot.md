# 2026-06-08-1351-after-hours-autopilot
...
- 결과: scheduler-owned `1351` core/research preflight를 우선 사용했고 Alpaca core `first_blocking_gate=market_closed`는 after-hours expected nonblocking으로 처리했다. 이번 cycle은 same-session after-hours order budget이 이미 `2/2`라 submit 없이 종료했다.

## 사용한 스케줄러 프리플라이트

- Alpaca core: `wiki/evidence-store/sources/2026-06-08-1351-after-hours-autopilot-alpaca-core-preflight.json`
- Research MCP: `wiki/evidence-store/sources/2026-06-08-1351-after-hours-autopilot-research-mcp-preflight.json`
- Alpaca core preflight의 `first_blocking_gate=market_closed`는 장외 워크플로우에서 예상되는 상태였고, passing account/positions/open-order/activity/asset rows는 그대로 사용했다. runtime Alpaca MCP `get_clock`, `get_account_info`, `get_all_positions`, `get_orders(status=open)`, `get_orders(status=all, after=2026-06-08T00:00:00Z)`, `get_account_activities(activity_types=FILL, after=2026-06-08T00:00:00Z)`, `get_watchlists`, `get_stock_latest_quote(feed=overnight)`, `get_asset(AVGO)`, `get_asset(QQQ)`는 same-session budget과 현재 executable quote stack을 교차 확인하는 용도로만 사용했다.

## Alpaca MCP 확인

- Regular market: closed (runtime `get_clock` `2026-06-08T00:52:53.26738282-04:00`)
- Account/positions: runtime `get_account_info` 기준 account `ACTIVE`, portfolio value `98593.12 USD`, cash `30731.86 USD`, buying power `296437.88 USD`, positions `33`건이었다.
...
- `AVGO` sell/trim은 runtime overnight quote `388.90/390.31`, spread `0.361902%`로 이번 cycle에는 after-hours spread cap `0.25%`를 넘어 preferred sell path에서 탈락했다.
- `GOOGL`, `XOM`, `TSLA`는 runtime overnight quote/freshness/spread/notional cap을 통과해 fallback buy executable stack을 형성했다.
- `BAC`는 spread는 통과했지만 quote age가 5분 cap을 넘어 stale이었다.
- `PFE`, `WMT`는 runtime overnight spread cap을 넘겼다.
- `QQQ`, `SPY`, `SMH`는 1주 ask가 after-hours per-order notional cap `492.97 USD`를 초과했다.
...
- Order plan: `wiki/trade-ledger/orders/2026-06-08-1351-after-hours-autopilot.json`
- Run manifest: `wiki/evidence-store/run-manifests/2026-06-08-1351-after-hours-autopilot.json`
- Gate evaluation: `wiki/evidence-store/sources/2026-06-08-1351-after-hours-autopilot-after-hours-gate-evaluation.json`
- Post-trade snapshot: `wiki/trade-ledger/positions/2026-06-08-1351-after-hours-autopilot-post-trade.json`
- Deterministic submit artifact: `wiki/evidence-store/sources/2026-06-08-1351-after-hours-autopilot-deterministic-submit.json`
