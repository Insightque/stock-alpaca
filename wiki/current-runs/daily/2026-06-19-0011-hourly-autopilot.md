# 2026-06-19-0011-hourly-autopilot scheduled paper autopilot
...
| Market clock | PASS | live `get_clock` `2026-06-18T11:14:08.556931184-04:00`, regular market open |
...
| Quote freshness | PASS | live quote timestamp `2026-06-18T15:14:25.716405974Z`, decision time 기준 약 `0.0`분 |
| Spread | MIXED | `RGTI 0.0483%`, `SO 0.0319%` PASS, buy fallback은 backlog throttle로 미진입 |
| Risk plan | PASS | `RGTI sell 3 @ 20.68 USD` trim plan 작성 |
...
| Symbol | 판단 | Spread | 이유 |
| --- | --- | ---: | --- |
| RGTI | executable_sell_trim | 0.0483% | speculative loss-control trim trigger, negative expected excess, open-order clean |
| SO | blocked_sell_metric_gap | 0.0319% | quote/spread는 pass지만 trim decision-grade expected-excess/replacement margin 공백 |
| AAPL | blocked_review_backlog_throttle | 0.0134% | buy-only backlog throttle로 신규 add 차단 |
...
| NVDA | blocked_review_backlog_and_same_theme_warning | 0.0191% | buy-only backlog throttle와 ai_semiconductor warning context가 겹침 |
...
- Planned orders: 1
- Submitted orders: 1
- Filled orders: 없음. `RGTI sell 3 @ limit 20.68 USD`는 immediate reconciliation 기준 아직 `status=new` open order다.
- Post-trade reconciliation: `get_order_by_client_id` 기준 `status=new`, `filled_qty=0`, `get_orders(status=open)` 기준 open orders `1`, `get_all_positions` 기준 positions `33`, `RGTI qty=15`, `qty_available=12`, `get_account_info` 기준 cash `28,301.48 USD`, portfolio value `101,530.37 USD`, buying power `302,809.21 USD`, `get_watchlists` 기준 watchlists `0`
...
| `check-risk-policy.py --json` | PASS | `RGTI sell 3 @ 20.68 USD` trim plan |

## 산출물

- Report: `wiki/current-runs/daily/2026-06-19-0011-hourly-autopilot.md`
- Manifest: `wiki/evidence-store/run-manifests/2026-06-19-0011-hourly-autopilot.json`
- Order plan: `wiki/trade-ledger/orders/2026-06-19-0011-hourly-autopilot.json`
- Runtime gate evaluation: `wiki/evidence-store/sources/2026-06-19-0011-hourly-autopilot-runtime-gate-evaluation.json`
- Post-trade snapshot: `wiki/trade-ledger/positions/2026-06-19-0011-hourly-autopilot-post-trade.json`
