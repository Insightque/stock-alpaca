...
| `장외 자동 운영해줘` 또는 `장외 주문 테스트해줘` | `harness/workflows/after-hours-autopilot.md` 또는 explicit wrapper probe | 조건부 제출 | 분리된 장외 정책 profile, artifact tag, review bucket |
...
| `2026-05-22 기준 추천 시뮬레이션해줘` | `harness/workflows/historical-decision-sim.md` | 없음 | 과거 시점 기준 추천, dry-run 주문 계획, 회고 예정일 |
| `2026-05-22 추천 회고해줘` | `harness/workflows/historical-decision-review.md` | 없음 | 1D/5D/20D 성과 평가, 정책학습 신호 |
...
Alpaca MCP로 계좌, 현금, buying power, 포지션, 미체결 주문을 확인하고 `wiki/trade-ledger/positions/current.md`를 업데이트합니다. 새 주문은 만들지 않습니다.
...
### `장외 자동 운영해줘` / `장외 주문 테스트해줘`

정규장 자동운영과 섞지 않습니다. `harness/recommendation-policy.yaml`의 `after_hours_policy`를 사용하고, 주문 계획에는 `market.session=after_hours`, 주문에는 `extended_hours=true`, `session=after_hours`, 장외 전용 `review_bucket`을 기록합니다. 주문 예산과 회고도 정규장 validation과 분리합니다.
...
실제 체결된 paper 거래와 보유/청산 결과를 과거 분석 기록과 비교합니다. 당시 thesis, 점수, 주문 계획, 시장 맥락, raw source를 근거로 무엇을 잘 판단했고 무엇을 놓쳤는지 기록합니다. 결과는 `wiki/trade-ledger/reviews/`에 남기고, 반복적으로 확인된 교훈만 `wiki/policy-book/recommendation-policy.md`에 반영합니다. 새 주문은 절대 제출하지 않습니다.

### `YYYY-MM-DD 기준 추천 시뮬레이션해줘`

해당 과거 날짜 또는 시각 기준으로 당시 알 수 있었던 정보만 사용해 추천을 재생성합니다. 날짜만 주어지면 해당 미국 정규 거래일 종가 기준으로 봅니다. 결과는 `wiki/backtest-runs/decisions/`에 저장하고, 매수/매도 수량은 dry-run 주문 계획으로만 남깁니다. 실제 주문은 절대 제출하지 않습니다.

### `YYYY-MM-DD 추천 회고해줘`

해당 과거 추천 시뮬레이션을 1D/5D/20D 이후 실제 가격으로 평가합니다. 미래 가격은 `wiki/backtest-runs/results/`의 회고 문서에만 기록하고, 원 추천 문서는 수정하지 않습니다. 반복적으로 확인된 교훈만 추천 정책에 반영합니다.
