...
해당 과거 날짜 또는 시각 기준으로 당시 알 수 있었던 정보만 사용해 추천을 재생성합니다. 날짜만 주어지면 해당 미국 정규 거래일 종가 기준으로 봅니다. 결과는 `wiki/simulations/`에 저장하고, 매수/매도 수량은 dry-run 주문 계획으로만 남깁니다. 실제 주문은 절대 제출하지 않습니다.

### `YYYY-MM-DD 추천 회고해줘`

해당 과거 추천 시뮬레이션을 1D/5D/20D 이후 실제 가격으로 평가합니다. 미래 가격은 `wiki/reviews/decisions/`의 회고 문서에만 기록하고, 원 추천 문서는 수정하지 않습니다. 반복적으로 확인된 교훈만 추천 정책에 반영합니다.
